...
import { PropertyStatus } from 'apps/nestar-api/src/libs/enums/property.enum (1)';
...
export class BatchService {
  ...
  public async batchTopProperties(): Promise<void> {
    ...
    const promisedList = properties.map(async (ele: Property) => {
      ...
      const rank = (propertyLikes ?? 0) * 2 + (propertyViews ?? 0) * 1;
  ...
  public async batchTopAgents(): Promise<void> {
    ...
    const promisedList = agents.map(async (ele: Member) => {
      ...
      const rank = (memberProperties ?? 0) * 5 + (memberArticles ?? 0) * 3 + (memberLikes ?? 0) * 2 + (memberViews ?? 0) * 1;
